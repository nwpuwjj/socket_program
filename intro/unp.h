...
#ifdef HAVE_POLL_H
#include <pool.h>   /*for convenience*/
#endif 
...
#ifndef HAVE_BZERO
#define bzero(ptr,n)  mmset(ptr,0,n)
#endif 
...
#endif 
#define __SS_PAD2SIZE (__SS_ALIGNSIZE - 2*__SS_ALIGNSIZE)
...
#define FILE_MODE (S_ISUSR | S_IWUSR | S_IRGRP | S_IROTH) /*default file access permissions for new files */
#define DIR_MODE (FILE_MODE | S_IXUSR | S_IXGRP | X_IXOTH) /*default permissions for new directories*/
...
#define err_sys(format,...)   printf(format, ##__VA_ARGS__); fflush(stdout);
#define err_quit(format,...)  printf(format, ##__VA_ARGS__); fflush(stdout);exit(-1);
